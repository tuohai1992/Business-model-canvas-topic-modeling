...
def top2vec(path_):
    ...
    for embedding in embeddings:
        ...
        data_fr['id'] = data_fr.index
        # Get citation

        data = data_fr['Value Propositions'].tolist()
        ...
        plt.xticks(fontsize=15)  # Increase x-axis content font size
        plt.yticks(fontsize=6)  # Increase y-axis content font size
        fig = plt.gcf()
        ...
        topic_words_save = pd.DataFrame(list(zip(topic_nums,topic_words,topic_words_short,word_scores)),columns=['Topic','topic_words','topic_words_short','word_scores'])
        ...
        top= np.argpartition(euclidean_distance.ravel(), -70)[-70:]
        ...
        docs_df['cik_str'] = data_fr['cik']
        docs_df['title'] = data_fr['companyName']
        docs_df['HPC_healthcare'] = data_fr['Judge']
        docs_df['business_report'] = data
        #docs_df['topic_words_short']=topic_words_short
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type = str,
        default = "data/10K_report_HPC_judge_BM_cleaned_all_final_value_proposition_all.csv",
